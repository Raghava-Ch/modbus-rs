...
fn main() -> Result<()> {
    ...
    let host = if args.len() > 1 { &args[1] } else { "192.168.55.105" };
    let port = if args.len() > 2 { args[2].parse().unwrap_or(502) } else { 502 };
    ...
    // Configure Modbus
    let mut config = ModbusConfig::new(host, port).map_err(|e| anyhow::anyhow!(MbusError::from(e)))?;
    ...
    client.read_device_identification(1, unit_id, ReadDeviceIdCode::Basic, ObjectId::from(0x00))
        .map_err(|e| anyhow::anyhow!(e))?;
    client.poll();
    ...
    client.read_device_identification(2, unit_id, ReadDeviceIdCode::Regular, ObjectId::from(0x00))
        .map_err(|e| anyhow::anyhow!(e))?;
    client.poll();
    ...
    client.read_device_identification(3, unit_id, ReadDeviceIdCode::Extended, ObjectId::from(0x80))
        .map_err(|e| anyhow::anyhow!(e))?;
    client.poll();
    ...
    client.read_device_identification(4, unit_id, ReadDeviceIdCode::Specific, ObjectId::from(0x00))
        .map_err(|e| anyhow::anyhow!(e))?;
    client.poll();
