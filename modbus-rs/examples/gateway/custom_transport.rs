...
use modbus_rs::mbus_core::transport::{ModbusConfig, Transport, TransportType, UnitIdOrSlaveAddr};
...
fn main() {
    ...
    // downstream bus.
    let mut gw: GatewayServices<LoopbackTransport, LoopbackTransport, _, _> =
        GatewayServices::new(PassthroughRouter, NoopEventHandler, 1000);
    gw.add_upstream(upstream).expect("upstream registered");
    gw.add_downstream(DownstreamChannel::new(downstream))
        .expect("channel 0 registered");

    // ── One poll cycle ────────────────────────────────────────────────────────
    gw.poll(0); // pass 0 as now_ms since timing is not used for this instant loopback

    // ── Inspect what the upstream client received ─────────────────────────────
    let upstream_sent = gw
        .upstream(0)
        .unwrap()
        .transport()
        .pending
