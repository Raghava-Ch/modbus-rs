...
use modbus_rs::{
    ...
    SerialMode, StdAsciiTransport, UnitIdOrSlaveAddr,
};
use std::str::FromStr;
use std::thread;
...
fn parse_cli() -> Result<(String, u8, u32)> {
    ...
    {
        println!("Usage: server_serial_ascii_demo [--port PATH] [--unit UNIT] [--baud BAUD]");
        std::process::exit(0);
    }
...
fn main() -> Result<()> {
    ...
    println!("Modbus ASCII server running on {}", port_path);
    println!("Unit id: {}", unit_raw);
    println!("Baud: {}", baud_raw);
    println!("Framing: 7E1, SerialMode::Ascii");
