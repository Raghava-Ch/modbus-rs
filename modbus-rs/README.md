...
[dependencies]
# Sync TCP server
modbus-rs = { version = "0.15.0", features = ["server", "network-tcp", "coils", "holding-registers"] }

# Async TCP server
modbus-rs = { version = "0.15.0", features = ["server", "async", "network-tcp", "coils", "holding-registers"] }
...
modbus-rs = { version = "0.15.0", features = ["gateway", "network-tcp", "serial-rtu"] }

# Async gateway: TCP upstream → TCP downstream
modbus-rs = { version = "0.15.0", features = ["gateway", "async", "network-tcp"] }
tokio = { version = "1", features = ["full"] }
```
...
```toml
[dependencies]
modbus-rs = { version = "0.15.0", default-features = false, features = ["network-tcp", "logging"] }
env_logger = "0.11"
```
...
```toml
[dependencies]
mbus-ffi = { version = "0.15.0", default-features = false, features = ["wasm", "full"] }
```
