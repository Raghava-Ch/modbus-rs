//! .NET P/Invoke surface for [`mbus_client_async::AsyncSerialClient`].
//!
//! Mirrors `tcp.rs` but wraps the serial (RTU / ASCII) client.  Two
//! constructors are provided — `mbus_dn_serial_client_new_rtu` and
//! `mbus_dn_serial_client_new_ascii` — that accept plain C strings for the
